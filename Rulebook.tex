...
\footnotetext[2]{Berk Calli et. al. \textit{Benchmarking in Manipulation Research: The YCB Object and Model Set and Benchmarking Protocols}. IEEE Robotics and Automation Magazine, 2015.}

\footnotetext[3]{Young Sang Choi et. al. (2009), \textit{A list of household objects for robotic retrieval prioritized by people with ALS}. IEEE International Conference on Rehabilitation Robotics, ICORR 2009.}
...
\textbf{Boss-character}. During the competitions, one local and representative object may be used to encourage teams to solve some of the most difficult tasks while making the competition more exciting for viewers.\\
...
\begin{itemize}
    \item Penalty points in \textbf{Clean Up} can be given in the following scenarios:
        ...
        \SubItem{If the robot is stuck during the competition, it can be restarted if necessary. After the robot has been stopped by the operator, the robot should be operated and initialised in the starting point, limited to one restart (to prevent repeatedly fine adjustments): Points x 50\% in the next object.}
